...
As of `r day` the COVID-19 radar application got data for `r npatient` users, `r nfemale` of whom were females (`r pfemale`%) and `r nmale` males (`r pmale`%).
...
`r nrow(waiting)` users are still waiting for the results of the test.
...
`r sum(!is.na(assessment$fever) & assessment$fever == "True")` instead reported of having fever -- albeit not everyone of them reported their temperature:
